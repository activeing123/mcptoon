...
- 版本：v0.7.21 · 1264 passed + 1 skipped · 0 依赖 · 206KB wheel · 27 模块 · Apache 2.0
...
python -m pytest tests/ -v          # 1264 tests, green expected

docker run --rm -v ~/.mcptoon:/root/.mcptoon mcptoon manifest --compact
```

零第三方导入是 review 阶段硬性规则。新功能必须带测试。
17,574 行 Python（物理行）、27 模块。见 [CONTRIBUTING.md](CONTRIBUTING.md)。

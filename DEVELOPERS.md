...
- 版本：v0.7.17 · 1036 passed + 1 skipped · 0 依赖 · 180KB wheel · 24 模块 · Apache 2.0
...
python -m pytest tests/ -v          # 1036 tests, green expected

docker run --rm -v ~/.mcptoon:/root/.mcptoon mcptoon manifest --compact
```

零第三方导入是 review 阶段硬性规则。新功能必须带测试。
15,320 行 Python（物理行）、24 模块。见 [CONTRIBUTING.md](CONTRIBUTING.md)。

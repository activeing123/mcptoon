...
- 版本：v0.8.0 · 1471 passed + 1 skipped · 0 依赖 · 227KB wheel · 28 模块 · Apache 2.0
...
python -m pytest tests/ -v          # 1471 tests, green expected

docker run --rm -v ~/.mcptoon:/root/.mcptoon mcptoon manifest --compact
```

零第三方导入是 review 阶段硬性规则。新功能必须带测试。
20,170 行 Python（物理行）、28 模块。见 [CONTRIBUTING.md](CONTRIBUTING.md)。
